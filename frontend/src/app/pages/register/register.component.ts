...
@Component({
  ...
  template: `
    <div class="auth-page">
        ...
        @for (planet of planets; track planet.name) {
          <div class="orbit" [style.width.px]="planet.orbit" [style.height.px]="planet.orbit"
            ...
            <div class="planet" [style.background]="planet.color" [style.width.px]="planet.size"
                 [style.height.px]="planet.size"></div>
          </div>
        }
      ...
      <div class="auth-card">
        ...
        <p class="subtitle">Create an account to share your astrological insights</p>
        ...
        <form #regForm="ngForm" (ngSubmit)="onRegister(regForm)">
          <div class="form-field">
            ...
            <input type="text" [(ngModel)]="fullName" name="fullName" #nameRef="ngModel"
                   placeholder="Your full name" required minlength="2" maxlength="100"
                   [class.invalid]="nameRef.touched && nameRef.invalid" />
            @if (nameRef.touched && nameRef.errors) {
              <span class="field-error">
                @if (nameRef.errors['required']) { Full name is required }
                @else if (nameRef.errors['minlength']) { Name must be at least 2 characters }
              </span>
            }
          </div>
          <div class="form-field">
          ...
          <div class="form-field">
            ...
            @if (passRef.touched && passRef.errors) {
              <span class="field-error">
                @if (passRef.errors['required']) { Password is required }
                @else if (passRef.errors['minlength']) { Password must be at least 6 characters }
              </span>
            }
            @if (passRef.valid && password) {
              <div class="password-strength">
                ...
                <span class="strength-label" [class.weak]="passwordStrength === 'Weak'"
                      [class.medium]="passwordStrength === 'Medium'"
                      [class.strong]="passwordStrength === 'Strong'">{{ passwordStrength }}</span>
              </div>
            }
          </div>
          <div class="form-field">
            <label>Confirm Password</label>
            <input type="password" [(ngModel)]="confirmPassword" name="confirmPassword" #confirmRef="ngModel"
                   placeholder="Repeat password" required
                   [class.invalid]="confirmRef.touched && confirmPassword && password !== confirmPassword" />
            @if (confirmRef.touched && confirmPassword && password !== confirmPassword) {
          ...
          <button type="submit" class="btn-primary full-width" [disabled]="loading || regForm.invalid || password !== confirmPassword">
  ...
  styles: [`
    ...
    .btn-google {
      ...
      &:hover { background: var(--jj-border); }
    }
    ...
    .strength-bar {
      ...
      &.weak .strength-fill { width: 33%; background: var(--jj-danger); }
      &.medium .strength-fill { width: 66%; background: var(--jj-accent); }
      &.strong .strength-fill { width: 100%; background: var(--jj-success); }
    }
    ...
    .strength-label {
      ...
      &.weak { color: var(--jj-danger); }
      &.medium { color: var(--jj-accent); }
      &.strong { color: var(--jj-success); }
    }
